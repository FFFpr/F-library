...
```
 --headless --generate-mono-glue modules/mono/glue
```
...
should be of the form bin/godot..editor..mono, e.g.
...
- On Windows, C:\Users\\MyLocalNugetSource
- On Linux, *BSD, etc., ~/MyLocalNugetSource

This path is referred to later as .

After picking a directory, run this .NET CLI command to configure NuGet to use
your local source:

```
dotnet nuget add source  --name MyLocalNugetSource
```

When you run the build_assemblies.py script, pass  to
...
```
./modules/mono/build_scripts/build_assemblies.py --godot-output-dir ./bin --push-nupkgs-local  --precision=double
```
...
./modules/mono/build_scripts/build_assemblies.py --godot-output-dir ./bin --push-nupkgs-local  --godot-platform=windows
```
...
# Generate binaries
./modules/mono/build_scripts/build_assemblies.py --godot-output-dir ./bin --push-nupkgs-local  --godot-platform=linuxbsd
```
...
directory may be placed in the .app/Contents/Resources/
...
and  is the current architecture of the export.
