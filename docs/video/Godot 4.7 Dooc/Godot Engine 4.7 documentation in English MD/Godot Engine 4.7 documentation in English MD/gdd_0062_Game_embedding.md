...
embedded game. Clicking the Game main screen button focuses the
floating window.
...
the Game main screen button switches to the tab with the running
project.
...
![Accessing the Game embedding main screen](../../_images/game_embedding_main_screen.png)
...
![Game embedding mode dropdown](../../_images/game_embedding_mode_dropdown.png)
...
If both display/window/size/window_width_override
...
![Game bar at the top of the embedded game window](../../_images/game_embedding_game_bar.png)
...
Engine.time_scale at runtime, it will
be multiplied by the value set here.

![Game speed dropdown on the left of the Game bar](../../_images/game_embedding_game_speed_dropdown.png)
...
The maximum number of physics steps per frame
...
is running instead. Make sure Debug > Synchronize Scene Changes is enabled
when doing this.
...
mode by using Ctrl + Alt + Right mouse button at the desired location.
...
Space + Left mouse button) to pan around and the mouse wheel to zoom.
- In the 3D interaction mode, hold the right mouse button and press W,
A, S, and D to use freelook. Use the middle mouse button to
orbit, Shift + Middle mouse button to pan, and the mouse wheel to zoom.
Additionally, you can use Ctrl + Minus, Ctrl + Plus, and Ctrl + 0
...
bar by unchecking Keep on Top using PiP in the menu on the right of
the Game bar.
- Window mode changes (e.g. fullscreen) are not supported when using game embedding.
- When Debug > Customize Run Instances... is used to enable running with
