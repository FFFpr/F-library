...
When a Container-derived node is used, all children Control nodes give up their
...
(via HSplitContainer and VSplitContainer).
...
(via HFlowContainer and via VFlowContainer).
...
it as if it was an image (via SubViewportContainer).
