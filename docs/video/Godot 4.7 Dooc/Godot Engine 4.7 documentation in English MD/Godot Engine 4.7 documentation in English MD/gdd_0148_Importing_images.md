...
- BMP (.bmp) - All pixel formats are supported, but RLE
...
libktx [https://github.com/KhronosGroup/KTX-Software]. Only supports 2D images.
...
See Changing import type for details.
Godot uses the ThorVG [https://www.thorvg.org/] library for SVG rendering.
SVG feature support is limited [https://www.thorvg.org/about#:~:text=certain%20features%20remain%20unsupported%20within%20the%20current%20framework];
...
to PNGs using Inkscape [https://inkscape.org/] is often a better solution.
...
without converting them back to an Image in a script. This is what
...
![Import options in the Import dock after selecting an image in the FileSystem dock](../../_images/importing_images_import_dock.png)
...
TextureButton and TouchScreenButton). This resource
type cannot be displayed directly onto 2D or 3D nodes, but the pixel values
can be queried from a script using get_bit.
...
to be reimported.
See Font and image oversampling for details.
- Font Data (Monospace Image Font): Import the image as a bitmap font where
all characters have the same width. See Using Fonts.
...
texture arrays, so you must create a custom shader in 2D
or 3D to display a texture from a texture array.
...
FogMaterial density maps in volumetric fog, particle attractor
vector fields, Environment 3D LUT color correction, and custom shaders.
...
BaseMaterial3D). If this happens, several import options are
...
Detect 3D > Compress To is changed. The texture is
...
Detect 3D > Compress To option before using the
texture in 3D, or change Compress > Mode to
...
per-material configuration in 3D.
In custom shaders, filter and repeat mode is changed on the sampler2D
uniform using hints described in the Shading language
documentation.
...
![Previewing a texture in the Inspector](../../_images/importing_images_inspector_preview.png)

Previewing a texture in the Inspector. Credit: Red Brick 03 - Poly Haven [https://polyhaven.com/a/red_brick_03]
...
If enabled, uses BPTC compression on desktop platforms and ASTC compression on mobile platforms. When using BPTC,
...
don't look that nice in normal maps, the RGTC
compression format is the best fit for this data. Forcing this option to Enable
will make Godot import the image as RGTC compressed.
...
Note that RGTC compression affects the
...
In the example below, the normal map with RGTC
...
![Normal map with standard VRAM compression (left) and with RGTC VRAM compression (right)](../../_images/importing_images_normal_map_rgtc.png)
...
textures (Cubemap, CubemapArray, Texture2DArray
and Texture3D). This forces all layers from the texture to be imported
with the RG color format, with only the red and green
channels preserved. RGTC compression is able to
...
![../../_images/fixedborder.png](../../_images/fixedborder.png)
...
This changes the Compress > Mode option that is used
...
Godot's own color palette for editor icons, with the
...
To support multiple resolutions with crisp
