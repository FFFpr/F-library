...
```
double time = GetNode("Player").GetPlaybackPosition() + AudioServer.GetTimeSinceLastMix();
```
...
```
double time = GetNode("Player").GetPlaybackPosition() + AudioServer.GetTimeSinceLastMix() - AudioServer.GetOutputLatency();
```
