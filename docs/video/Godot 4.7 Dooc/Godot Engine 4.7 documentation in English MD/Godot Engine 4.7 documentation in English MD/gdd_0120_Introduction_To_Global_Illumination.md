...
![../../../_images/global_illumination_example.png](../../../_images/global_illumination_example.png)
...
(SSAO) can be enabled in the environment
...
Color banding section in the 3D rendering
...
can be enabled in the environment settings.
...
![A 3D scene without any form of global illumination (only constant environment lighting). The box and sphere near the camera are both dynamic objects.](../../../_images/gi_none.png)
...
- For desktop games, it's a good idea to start with SDFGI
...
graphics, consider adding an option to disable SDFGI or VoxelGI in your game's settings. SDFGI can be disabled in the
...
- For mobile games, LightmapGI and
ReflectionProbes are the only supported options.
See also Alternatives to GI techniques.
...
simulate GI by placing additional lights manually.
