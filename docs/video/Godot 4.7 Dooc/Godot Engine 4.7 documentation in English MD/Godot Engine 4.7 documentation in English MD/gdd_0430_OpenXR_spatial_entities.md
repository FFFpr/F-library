...
![../../_images/openxr_spatial_entities_project_settings.png](../../_images/openxr_spatial_entities_project_settings.png)
...
object is instantiated and registered with the XRServer.
...
As the tracker objects can be used with XRAnchor3D nodes, these subscenes
...
object. As all locations are local to the XROrigin3D node, we should create
...
Spatial anchors are tracked using OpenXRAnchorTracker objects
registered with the XRServer.

When needed, the location of the spatial anchor will be updated automatically; the pose on the
related tracker will be updated and thus the XRAnchor3D node will
reposition.
...
This scene should have an XRAnchor3D node as the root but nothing
...
The root node for this scene must be an XRAnchor3D node.
We'll add a StaticBody3D node as a child and add a
CollisionShape3D and MeshInstance3D
node as children of the static body.

![../../_images/openxr_plane_anchor.png](../../_images/openxr_plane_anchor.png)
...
For our "hole punch" material, create a ShaderMaterial
...
Please consult the OpenXRPlaneTracker class documentation for
further information.
...
When markers are detected, OpenXRMarkerTracker objects are
...
with an XRAnchor3D node at the root, save this, and assign it to the
...
OpenXRSpatialEntityExtension singleton.
...
entry and that is OpenXRSpatialQueryResultData.
...
OpenXRSpatialEntityTracker instance is destroyed.

### Spatial anchor capability

Spatial anchors are managed by our OpenXRSpatialAnchorCapability
singleton object.
...
Finally we can process our snapshot. Note that we are using OpenXRAnchorTracker
...
We can use our OpenXRPlaneTracker tracker object to store our component data.
