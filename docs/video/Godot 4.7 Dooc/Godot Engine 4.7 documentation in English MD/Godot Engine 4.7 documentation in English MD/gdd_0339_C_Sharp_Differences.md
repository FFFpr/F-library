...
As explained in General differences between C# and GDScript, PascalCase is used
...
reasonably possible. See the C# style guide, which we encourage
...
are considered essential. See Configuring an external editor.
...
tan | Mathf.Tan
tanh | Mathf.Tanh
type_convert | Variant.As or GD.Convert
...
is called with @onready (cf. @onready annotation).
For example:
...
GDScript | C#
html(String) | FromHtml(ReadOnlySpan)
...
Godot.Collections.Array is a type-safe wrapper around Godot.Collections.Array.
...
Godot.Collections.Dictionary is a type-safe wrapper around Godot.Collections.Dictionary.
...
used with operands of the types Task [https://learn.microsoft.com/en-us/dotnet/api/system.threading.tasks.task], Task [https://learn.microsoft.com/en-us/dotnet/api/system.threading.tasks.task-1], ValueTask [https://learn.microsoft.com/en-us/dotnet/api/system.threading.tasks.valuetask], or ValueTask [https://learn.microsoft.com/en-us/dotnet/api/system.threading.tasks.valuetask-1].
