...
- Astar3DAstar3D objects provide an option to find the shortest path in a graph of weighted points.
...
- NavigationServer3DNavigationServer3D provides a powerful server API to find the shortest path between two positions on an area defined by a navigation mesh.
...
- NavigationRegion3D NodeA Node that holds a Navigation Mesh resource that defines a navigation mesh for the NavigationServer3D.
...
- NavigationLink3D NodeA Node that connects two positions on navigation meshes over arbitrary distances for pathfinding.
...
- NavigationAgent3D NodeA helper Node used to facilitate common NavigationServer3D API calls for pathfinding and avoidance.
Use this Node with a Node3D inheriting parent Node.
- NavigationObstacle3D NodeA Node that can be used to affect and constrain the avoidance velocity of avoidance enabled agents.
...
- NavigationMesh ResourceA resource that holds 3D navigation mesh data.
...
public partial class MyCharacterBody3D : CharacterBody3D
{
    ...
    {
        base._Ready();
        _navigationAgent = GetNode("NavigationAgent3D");
        // These values need to be adjusted for the actor's speed
        // and the navigation layout.
