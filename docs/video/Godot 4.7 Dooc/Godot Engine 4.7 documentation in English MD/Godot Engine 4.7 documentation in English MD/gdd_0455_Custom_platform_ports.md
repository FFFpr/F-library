...
Similar to Custom modules in C++, Godot's multi-platform architecture
...
it does not use the DisplayServer abstraction that is new in Godot 4.
...
similar, with the exception of DisplayServer not being split
from the OS singleton.
...
At the very least, a platform port must have methods from the OS
...
(for exporting from the editor, including One-click deploy).
...
One-click deploy is implemented for the target platform. This icon
...
Custom modules in C++, this allows for streamlining the build process
by making it possible to git clone a platform folder within a Godot repository
clone's platform/ folder, then run scons platform=. No other steps are
