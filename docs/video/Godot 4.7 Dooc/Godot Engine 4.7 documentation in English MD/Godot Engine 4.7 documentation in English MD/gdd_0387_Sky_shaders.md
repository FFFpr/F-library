...
Sky process mode is set to
PROCESS_MODE_REALTIME.

Note that the process mode only
...
be optimized by setting the sky's process mode
and the radiance size to get the
...
rollover
setting). It's affected by time_scale but not by pausing. If you need a
