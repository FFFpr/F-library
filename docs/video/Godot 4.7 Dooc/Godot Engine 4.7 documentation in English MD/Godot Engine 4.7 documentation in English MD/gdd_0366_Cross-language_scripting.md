...
(see Classes as resources), the script can be instantiated
with new().
...
explained in Type conversion and casting.
...
convoluted. You will have to use GodotObject.Get()
and GodotObject.Set(). The first argument is the name of the field you want to access.
...
Built-in types or classes extending Object.
...
GodotObject.Call(). The first argument is the
...
myGDScriptNode.Connect("my_signal_with_params", Callable.From(MySignalWithParamsHandler));
```
