...
shader_baker | Project was exported with shader baking
enabled (only applies to the exported project, not when running in the editor)
dedicated_server | Project was exported as a dedicated server
(only applies to the exported project, not when running in the editor)

Warning

With the exception of texture compression, web_ and
...
(including with One-click deploy). They are not used when
...
Custom feature tags are also not used in EditorExportPlugin
scripts. Instead, feature tags in EditorExportPlugin
...
![The Project Settings panel](../../_images/feature_tags2.png)
...
using the typical approaches (ProjectSettings.get_setting
or ProjectSettings.get).
Instead, you must use ProjectSettings.get_setting_with_override.
