...
and logo LICENSE.txt [https://github.com/godotengine/godot/blob/master/misc/logo/LICENSE.txt] files
...
For more on this, see the sections on exporting
and compiling Godot yourself.
...
you, check out the GDScript scripting guide.
...
languages can be found in the GDScript: An introduction to dynamic languages tutorial.
...
C++, using GDExtension, will almost always be
...
cross-language scripting, or by using
...
Importing 3D scenes documentation.
...
for additional languages, take a look at EditorPlugins
and tool scripts.
...
run the Godot editor from a terminal by entering godot.
...
See File paths in Godot projects for more information.

If true portable operation is desired (e.g. for use on a USB stick),
follow the steps in Self-contained mode.
...
aspect ratios works best. Check the Multiple resolutions tutorial
...
4. For user interfaces, use the anchoring
...
When it's ready! See When is the next release out? for more
information.
...
Which version should I use for a new project? for more information.
...
Should I upgrade my project to use new engine versions? for more information.
...
Godot does not use a standard GUI toolkit
...
This custom UI toolkit can be embedded into other applications
(experimental). However, the preferred way to use it is to
use Godot to create non-game applications with the editor.
