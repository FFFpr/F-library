...
A Vendor Runtime Module is a Godot module
which is only applicable at runtime in a running project.
It is created like a regular custom C++ module, but is
packaged using an editor plugin to make the functionality
...
Make sure to follow the instructions for creating a custom C++ module.
...
See the Compiling pages for more information.
...
accessible to end users via the Godot Asset Store [https://store.godotengine.org/].

Follow these instructions to start creating the plugin. Your base plugin script should look like the following:
...
The next step is to define and instantiate an EditorExportPlugin instance.
The EditorExportPlugin instance is used to hook into the export flow and
...
Feel free to explore and leverage those functionalities to improve the
