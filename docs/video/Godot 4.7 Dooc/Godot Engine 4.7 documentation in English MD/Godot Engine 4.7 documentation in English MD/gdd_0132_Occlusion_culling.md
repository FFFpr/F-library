...
![Example scene with an occlusion culling-friendly layout](../../_images/occlusion_culling_scene_example.png)
...
![Example scene with occlusion culling disabled (wireframe)](../../_images/occlusion_culling_disabled.png)
...
![Example scene with occlusion culling enabled (wireframe)](../../_images/occlusion_culling_enabled.png)
...
this with Mesh level of detail (LOD) and Visibility ranges (HLOD) to further improve
performance gains.
...
AABB against the occluder shapes.
The occludee's AABB must be fully occluded
...
the nodes themselves to move in global space, therefore requiring the BVH to be rebuilt.
...
occlusion culling debug draw mode to look
for occluder shapes that shouldn't be present, then
adjust the bake cull mask accordingly.
