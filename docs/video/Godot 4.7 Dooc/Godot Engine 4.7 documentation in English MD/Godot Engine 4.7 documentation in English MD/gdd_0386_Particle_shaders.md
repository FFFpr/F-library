...
CPU-based particle nodes (CPUParticles2D and
CPUParticles3D) are rendered on the GPU (which means they can
...
in float INTERPOLATE_TO_END | Value of the interp_to_end
(3D) property of the Particles node.
in uint AMOUNT_RATIO | Value of the amount_ratio
(3D) property of the Particles node.
