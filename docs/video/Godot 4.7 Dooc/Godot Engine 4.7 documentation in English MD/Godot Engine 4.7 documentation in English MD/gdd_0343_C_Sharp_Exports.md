...
see GDScript exported properties.
...
_PropertyCanRevert() and
_PropertyGetRevert() methods.
...
GD.Load and GetNode. See Nodes and Resources.
...
a clickable button. Since this runs in the editor, usage of the [Tool] attribute is required. When the button is
...
icon will be fetched via GetThemeIcon(),
...
As explained in the C# Variant documentation, only
...
Using the generic Godot.Collections.Array allows specifying the type of the
...
```csharp
[Export]
public Godot.Collections.Array CharacterNames { get; set; } =
[
    "Rebecca",
    "Mary",
    "Leah",
...
```csharp
[Export]
public Godot.Collections.Array Textures { get; set; }
[Export]
public Godot.Collections.Array Scenes { get; set; }
```
...
Using the generic Godot.Collections.Dictionary allows specifying
...
```csharp
[Export]
public Godot.Collections.Dictionary CharacterLives { get; set; } = new Godot.Collections.Dictionary
{
    ["Rebecca"] = 10,
    ["Mary"] = 42,
    ["Leah"] = 0,
...
_Set(),
_Get(), and
_GetPropertyList() methods as
described in Accessing data or logic from an object.
