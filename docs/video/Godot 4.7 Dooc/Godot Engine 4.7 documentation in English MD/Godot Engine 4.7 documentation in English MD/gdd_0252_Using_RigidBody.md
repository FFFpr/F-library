...
In order to define the shape of the body, it must have one or more Shape3D objects assigned. Note that setting the position of these shapes will affect the body's center of mass.
...
See RigidBody3D and PhysicsMaterial for
...
public partial class MyRigidBody3D : RigidBody3D
{
    ...
    {
        Vector3 targetPosition = GetNode("MyTargetNode3DNode").GlobalTransform.Origin;
...
This method can be used with axis_lock_angular_* as well. If more precise control is needed, solutions such as ones relying on Quaternion may be required,
as discussed in Using 3D transforms.
