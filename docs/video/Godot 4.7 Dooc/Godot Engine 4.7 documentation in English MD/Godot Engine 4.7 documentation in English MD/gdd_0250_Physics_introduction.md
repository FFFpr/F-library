...
Physics in Godot, regardless of physics engine, is not deterministic,
...
- Area2DArea2D nodes provide detection and influence. They can detect when
...
in a defined area.
- StaticBody2DA static body is one that is not moved by the physics engine. It participates
...
- RigidBody2DThis is the node that implements simulated 2D physics. You do not control a
...
Read more about using rigid bodies.
- CharacterBody2DA body that provides collision detection, but no physics. All movement and
collision response must be implemented in code.

### Physics material

Static bodies and rigid bodies can be configured to use a PhysicsMaterial. This allows adjusting the friction and bounce of an object,
...
![../../_images/player_coll_shape.png](../../_images/player_coll_shape.png)
...
differentiates between physics and idle processing.
...
- collision_layerThis describes the layers that the object appears in. By default, all
bodies are on layer 1.
- collision_maskThis describes what layers the body will scan for collisions. If an
...
![../../_images/physics_layer_names.png](../../_images/physics_layer_names.png)
...
![../../_images/player_collision_layers.png](../../_images/player_collision_layers.png)

![../../_images/player_collision_mask.png](../../_images/player_collision_mask.png)
...
contact reporting, set the max_contacts_reported
...
CharacterBody2D bodies detect collisions with
