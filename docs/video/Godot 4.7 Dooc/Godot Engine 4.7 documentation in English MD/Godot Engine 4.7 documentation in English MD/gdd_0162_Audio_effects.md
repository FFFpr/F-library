...
![../../_images/audio_buses4.png](../../_images/audio_buses4.png)
...
hard limiter or a compressor
to prevent clipping, or distortion on clip mode if
...
the notch filter, but weaker. It is the opposite of
the band-pass filter. This filter can be used to
...
band-limit filter and
notch filter. This filter can be used to simulate
...
altogether, a hard limiter is likely a better
...
reverb, which has a more natural and blurred sound to it.
...
If clipping is desired, consider using the distortion
effect on clip mode.
...
hard limiter effect instead. This effect is kept to
...
band-pass filter. This filter can be used to give
...
AudioStreamPlayer3D, since they handle
...
Stores audio data into an AudioStreamWAV.
...
similar to delay, which has a less blurry sound to it. Using
reverb in conjunction with delay can create very natural sounding environments.

Reverb is commonly outputted from Area3Ds
(see Reverb buses).
