...
using occluder nodes. See Occlusion culling for instructions on
...
Godot sorts objects by Material and Shader to improve performance. This, however, can not be done with
...
For more information, see the GPU optimizations
doc.
...
(for more information on fill rate, see GPU optimization).
...
global illumination are especially
...
See Using Lightmap global illumination for instructions on using baked lightmaps. For
...
and quality is to keep Dynamic for the DirectionalLight3D node,
...
The VisibleOnScreenEnabler3D and VisibleOnScreenNotifier3D
...
large worlds. This can be resolved using Large world coordinates.
