...
Label nodes are great for displaying basic text, but they have limitations.
...
of the text centered. To get around these limitations, you would use a RichTextLabel.
...
By default, RichTextLabel functions like a normal Label.
It has the text property, which you can
edit to have uniformly formatted text. To be able to use BBCode for rich text formatting,
you need to turn on the BBCode mode by setting bbcode_enabled.
...
![../../_images/bbcode_in_richtextlabel_inspector.png](../../_images/bbcode_in_richtextlabel_inspector.png)

For example, BBCode [color=green]test[/color] would render the word "test" with
a green color.

![../../_images/bbcode_in_richtextlabel_basic_example.png](../../_images/bbcode_in_richtextlabel_basic_example.png)
...
![Example of unescaped user input resulting in BBCode injection (2nd line) and escaped user input (3rd line)](../../_images/bbcode_in_richtextlabel_escaping_user_input.png)
...
Tag | Example
b
Makes {text} use the bold (or bold italics) font of RichTextLabel. | [b]{text}[/b]
i
Makes {text} use the italics (or bold italics) font of RichTextLabel. | [i]{text}[/i]
u
Makes {text} underlined. | [u]{text}[/u]
[u color={color}]{text}[/u]
s
Makes {text} strikethrough. | [s]{text}[/s]
[s color={color}]{text}[/s]
code
Makes {text} use the mono font of RichTextLabel. | [code]{text}[/code]
char
Adds Unicode character with hexadecimal UTF-32 {codepoint}. | [char={codepoint}]
p
Adds new paragraph with {text}. Supports configuration options,
see Paragraph options. | [p]{text}[/p]
[p {options}]{text}[/p]
br
...
see Horizontal rule options. | [hr]
[hr {options}]
center
Makes {text} horizontally centered.
Same as [p align=center]. | [center]{text}[/center]
left
Makes {text} horizontally left-aligned.
Same as [p align=left]. | [left]{text}[/left]
right
Makes {text} horizontally right-aligned.
Same as [p align=right]. | [right]{text}[/right]
fill
Makes {text} fill the full width of RichTextLabel.
Same as [p align=fill]. | [fill]{text}[/fill]
indent
Indents {text} once.
...
see URL options.
Must be handled with the "meta_clicked" signal to have an effect, see Handling  tag clicks. | [url]{link}
{text}
{text}
hint
...
surrounding text, see Image and table vertical alignment.
Supports configuration options, see Image options. | [img]{path}[/img]
[img={width}]{path}[/img]
...
Supports configuration options, see Font options. | [font={path}]{text}[/font]
[font {options}]{text}[/font]
font_size
Use custom font size for {text}. | [font_size={size}]{text}[/font_size]
dropcap
Use a different font size and color for {text}, while making the tag's contents
span multiple lines if it's large enough.
A drop cap [https://www.computerhope.com/jargon/d/dropcap.htm] is typically one
...
otherwise, the list won't be parsed correctly. | [opentype_features={list}]
{text}
[/opentype_features]
lang
...
#ff00ff, see Hexadecimal color codes). | [color={code/name}]{text}[/color]
bgcolor
...
outline_size
Use custom font outline size for {text}. | [outline_size={size}]
{text}
[/outline_size]
outline_color
Use custom outline color for {text}. Accepts same values as the color tag. | [outline_color={code/name}]
{text}
[/outline_color]
table
...
{name} is a table name for assistive apps (screen reader). | [table={number}]{cells}[/table]
...
Supports configuration options, see Cell options. | [cell]{text}[/cell]
...
see Unordered list bullet. | [ul]{items}[/ul]
...
control characters directly in the text. | [lrm] (left-to-right mark), [rlm] (right-to-left mark), [lre] (left-to-right embedding),
...
![../../_images/bbcode_in_richtextlabel_image_align.png](../../_images/bbcode_in_richtextlabel_image_align.png)
...
![../../_images/bbcode_in_richtextlabel_table_align.png](../../_images/bbcode_in_richtextlabel_table_align.png)
...
![../../_images/color_constants.png](../../_images/color_constants.png)
...
when the SceneTree is paused. You can change this
...
![../../_images/bbcode_in_richtextlabel_effect_pulse.png](../../_images/bbcode_in_richtextlabel_effect_pulse.png)
...
![../../_images/bbcode_in_richtextlabel_effect_wave.png](../../_images/bbcode_in_richtextlabel_effect_wave.png)
...
![../../_images/bbcode_in_richtextlabel_effect_tornado.png](../../_images/bbcode_in_richtextlabel_effect_tornado.png)
...
![../../_images/bbcode_in_richtextlabel_effect_shake.png](../../_images/bbcode_in_richtextlabel_effect_shake.png)
...
![../../_images/bbcode_in_richtextlabel_effect_fade.png](../../_images/bbcode_in_richtextlabel_effect_fade.png)
...
![../../_images/bbcode_in_richtextlabel_effect_rainbow.png](../../_images/bbcode_in_richtextlabel_effect_rainbow.png)
...
BBCode tags. Create a new script file that extends the RichTextEffect resource type
...
install_effect() method:

![Selecting a custom RichTextEffect after saving a script that extends RichTextEffect with a ``class_name``](../../_images/bbcode_in_richtextlabel_selecting_custom_richtexteffect.png)
