...
For all Objects, the most basic way of referencing them
...
The same principle applies for RefCounted objects.
...
public MyType
{
    ...
    public Script MyScript = GD.Load("res://Path/To/MyScript.cs");
    // Initialize with same value. Value cannot be changed.
    public readonly Script MyConstScript = GD.Load("res://Path/To/MyScript.cs");
    // Like 'readonly' due to inaccessible setter.
    // But, value can be set during constructor, i.e. MyType().
    public Script MyNoSetScript { get; } = GD.Load("res://Path/To/MyScript.cs");
...
public class MyNode : Node
{
    ...
    {
        MyNode globals = GetNode("/root/Globals");
        GD.Print(globals);
...
- Outsource the access to a Callable. These may be useful
