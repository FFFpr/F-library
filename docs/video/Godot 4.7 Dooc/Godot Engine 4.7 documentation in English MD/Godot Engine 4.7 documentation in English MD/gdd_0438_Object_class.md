...
OtherClass *obj2 = Object::cast_to(obj); // Converting between classes, similar to dynamic_cast
```
...
```cpp
protected:
     void _get_property_list(List *r_props) const;      // return list of properties
...
RefCounted subclasses are memory managed with
...
Ref my_ref = memnew(MyRefCounted);
// ...
...
void some_func(Object *p_object) {
     Button *button = Object::cast_to(p_object);
}
```
...
More information can be found on the Godot notifications page.
...
memory, the ResourceLoader will return that reference. This means that
