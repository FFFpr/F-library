...
Godot is a primarily C++ project and it uses the SCons build system.
...
If you want to adjust how many CPU threads SCons will use, use the -j
...
When creating builds for development (running debugging/profiling
...
continuous integration setup) and also builds unit tests so you can run them locally.
...
See Using sanitizers for more information.
...
time it takes to build the engine. See Optimizing a build for size page for more details.
