...
When the rendering/viewport/hdr_2d
...
rendering features such as SDFGI and Volumetric fog and fog volumes. Several
...
Rendering methods are supported when using the
Vulkan driver.
...
Both the Forward+ and Mobile Rendering methods can be
used with Direct3D 12.

Core shaders are shared with the
...
Core shaders are shared with the
Vulkan renderer. Shaders are transpiled from GLSL to MSL
using SPIRV-Cross.
...
See the pull request that introduced Metal 4 support [https://github.com/godotengine/godot/pull/114484]
for more information.
...
Only the Compatibility rendering
method can be used with the OpenGL driver.

Core shaders are entirely different
...
![../../_images/rendering_architecture_diagram.png](../../_images/rendering_architecture_diagram.png)
...
custom language inspired by GLSL, core shaders are
...
in LDR sRGB-space while 3D rendering uses
HDR linear space.
...
better precision [https://developer.nvidia.com/content/depth-precision-visualized],
...
Linearly Transformed Cosines [https://eheitzresearch.wordpress.com/757-2/]
technique.

### Shadow mapping

Both Forward+ and Mobile methods use
PCF to filter shadow maps and create a
...
pull request that overhauled SSR [https://github.com/godotengine/godot/pull/111210]
for more information.
