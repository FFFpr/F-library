...
```
./bin/ --test --source-file="*test_color*" --success
```
...
```
./bin/ --test --test-case-exclude="*[Stress]*"
```
...
module tests, refer to Writing custom unit tests.
...
```
./bin/ --test --source-file="*test_validate*" --success --reporters=xml --out=doctest.txt
```
...
```
./bin/ --test gdscript-tokenizer test.gd
./bin/ --test gdscript-parser test.gd
./bin/ --test gdscript-compiler test.gd
```
...
help of OS get_cmdline_args method.
...
bin/ --gdscript-generate-tests modules/gdscript/tests/scripts
