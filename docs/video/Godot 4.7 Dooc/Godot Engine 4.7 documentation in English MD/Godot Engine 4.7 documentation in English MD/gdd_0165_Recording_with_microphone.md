...
![../../_images/record_bus.png](../../_images/record_bus.png)

![../../_images/record_stream_player.png](../../_images/record_stream_player.png)
...
private void OnRecordButtonPressed()
{
    if (_effect.IsRecordingActive())
    {
        ...
        GetNode("PlayButton").Disabled = false;
        GetNode("SaveButton").Disabled = false;
        _effect.SetRecordingActive(false);
        GetNode("RecordButton").Text = "Record";
        GetNode("Status").Text = "";
    }
    else
    {
        GetNode("PlayButton").Disabled = true;
        GetNode("SaveButton").Disabled = true;
        _effect.SetRecordingActive(true);
        GetNode("RecordButton").Text = "Stop";
        GetNode("Status").Text = "Recording...";
    }
}
```
...
private void OnPlayButtonPressed()
{
    ...
    var audioStreamPlayer = GetNode("AudioStreamPlayer");
...
{
    string savePath = GetNode("SaveButton/Filename").Text;
    _recording.SaveToWav(savePath);
    GetNode("Status").Text = string.Format("Saved WAV file to: {0}\n({1})", savePath, ProjectSettings.GlobalizePath(savePath));
}
```
